...
type Player = { id: string; name: string; team_id: string | null; position: string | null };
    ...
    <div className="space-y-4">
      ...
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          <thead><tr className="text-xs text-zinc-500"><th className="text-left py-2">Ime</th><th className="text-left">Tim</th><th className="text-left">Pozicija</th><th></th></tr></thead>
          <tbody>
            {players.map((p) => (
              <tr key={p.id} className="border-t border-zinc-100">
                {editing === p.id ? (
                  ...
                  <>
                    ...
                    <td className="text-zinc-500">{p.team_id ? teamMap.get(p.team_id) : "—"}</td>
