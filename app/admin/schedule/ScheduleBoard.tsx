...
function MatchCard({ match, disabled }: { match: Match; disabled: boolean }) {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`bg-white border border-zinc-200 rounded-md p-2 text-xs ${disabled ? "opacity-60" : "cursor-grab hover:border-emerald-300"} ${isDragging ? "opacity-30" : ""}`}
    >
...
function RoundColumn({ round, children }: { round: Round; children: React.ReactNode }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`w-56 shrink-0 bg-zinc-50 rounded-lg p-2 border ${isOver ? "border-emerald-400 bg-emerald-50" : "border-zinc-200"}`}
    >
...
export function ScheduleBoard({ rounds, matches }: { rounds: Round[]; matches: Match[] }) {
  ...
  if (rounds.length === 0) {
    return <div className="card text-sm text-zinc-600">Još nema kola. Pokreni žreb iz <a href="/admin/draw" className="text-emerald-700 underline">/admin/draw</a>.</div>;
  }
  ...
  return (
    <DndContext sensors={sensors} onDragStart={(e: DragStartEvent) => setActiveId(e.active.id as string)} onDragEnd={onDragEnd} onDragCancel={() => setActiveId(null)}>
      ...
      <DragOverlay>
        {activeMatch ? (
          <div className="bg-white border border-emerald-400 shadow-lg rounded-md p-2 text-xs w-52">
            <div className="flex items-center gap-1 mb-1">
