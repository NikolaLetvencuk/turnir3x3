...
export default async function LiveMatchPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: match }, { data: events }, { data: players }] = await Promise.all([
    supabase.from("matches").select("*, home_team:teams!matches_home_team_id_fkey(id, name), away_team:teams!matches_away_team_id_fkey(id, name)").eq("id", params.id).maybeSingle(),
