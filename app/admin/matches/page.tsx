...
export default async function MatchesAdminPage() {
  ...
  const [{ data: matches }, { data: teams }, { data: rounds }, { data: groups }] = await Promise.all([
    supabase.from("matches").select("*, home:teams!matches_home_team_id_fkey(id,name), away:teams!matches_away_team_id_fkey(id,name), round:rounds(name, display_order)").order("kickoff_at"),
