...
import { Eye, Lock, Copy } from "lucide-react";
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs text-zinc-500">
              ...
              <th className="text-left">Igrač</th>
              {rounds.map((r) => (
                <th key={r.id} className="text-right px-2 whitespace-nowrap" title={r.status}>
                  <span className={r.status === "finished" ? "" : "text-zinc-400"}>{r.name}</span>
                </th>
              ))}
          ...
          <tbody>
            {members.map((m, i) => {
              ...
              return (
                <tr key={m.user_id} className={`border-t border-zinc-100 ${isMe ? "bg-emerald-50/40" : ""}`}>
                  ...
                  <td className="py-2">
                    <button onClick={() => setViewing(m)} className="inline-flex items-center gap-1.5 font-medium hover:text-emerald-700">
                      <span className="truncate max-w-[160px]">{m.display}</span>
                      {isMe && <span className="text-[10px] text-emerald-700 font-semibold">(ti)</span>}
                      <Eye className="w-3.5 h-3.5 text-zinc-400" />
                    </button>
                  </td>
                  {rounds.map((r) => (
                    <td key={r.id} className="text-right px-2 tabular-nums text-xs">
                      {m.per_round[r.id] != null ? m.per_round[r.id] : <span className="text-zinc-300">—</span>}
                    </td>
                  ))}
            ...
            {members.length === 0 && <tr><td colSpan={rounds.length + 3} className="py-4 text-center text-zinc-500">Nema članova.</td></tr>}
          </tbody>
        </table>
      </div>
